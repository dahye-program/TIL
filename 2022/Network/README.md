...
- 국제 표준 기구 ISO에서 지정한 컴퓨터 프로토콜 네트워킹에 관한 표준
- 인터넷 환경에서 통신하기 위한 통신 방식에 대한 약속 ⇒ 프로토콜
- 프로토콜을 7계층으로 묶어 통신을 체계화

### 물리 계층(Physical Layer) - 허브, 리피터 (bit)

- 실제 장치들을 연결하기 위해 필요한 물리적 세부사항 정의
- 허브는 이더넷으로 연결된 수많은 각각의 네트워크를 하나로 묶어줌
- 허브는 `플로딩(Flooding) 작업` 수행
- 하나의 PC에서 허브로 데이터가 보내지면, 허브는 모든 PC에게 데이터를 전송, 빠른 송신을 위함
- 허브에서 목적지에 대한 조사나 원하는 곳으로만 데이터를 전송하면 작업 비용이 매우 높음
- PC들은 자신에게 온 데이터라면 받아들이고, 아니라면 무시하게 됨
- 또한, 물리적 전기 신호가 목적지까지 안전하게 갈 수 있도록 리피터도 `플로딩(Flooding) 작업` 수행, 하나의 리피터에 연결된 모든 장비는 같은 데이터를 듣게 됨
- 데이터 송신 길이가 길어짐에 따라 물리적 신호가 점차 약해지게 되고 최악의 경우 데이터를 받는 쪽에서 알아챌 수 없을 정도의 약한 신호가 되어 데이터 전송이 실패할 수 있음
- 높은 산에 통신을 연결하기 위해 통신사에서 세워 놓은 증폭 장치도 일종의 리피터

### 데이터링크 계층(Data Link Layer) - 스위치 (frame)

- 물리 계층에서 전기 신호를 플로딩으로 전송하였고, 수신자는 자신의 데이터인지 판단
- 신호를 `Frame` 으로 변환
- `Frame` 에는 `MAC Address` 라는 고유의 주소 존재
  - 컴퓨터나 서버를 사면 네트워크 카드 존재
  - 네트워크 카드가 가지고 있는 유일한 물리적 주소가 `MAC Address`
- 스위치가 전기적 신호를 받아 `MAC Address` 를 보고 수신자에게 전달
- 또한, 물리 계층에서 발생한 오류 감지
  - 물리 계층에서는 신호 내용에 대한 체크를 하지 않기 때문에 신호 증폭에 실패해서 데이터를 받지 못하더라도 이를 알 방법이 없음
  - 물리 계층에서 데이터 오류가 발생하면 이를 데이터링크 계층에서 알아차리고 `MAC Address` 에 써있는 주소에게 말해 데이터를 재전송하라고 요청해야 함

### 네트워크 계층(Network Layer) - 라우터 (packet)

- 라우터는 이더넷을 통해 인터넷에 들어온 곳을 출발지로, 어떤 노드를 거쳐 목적지까지 도달할지 흐름을 제어
- 라우터에 도달한 정보들은 라우터가 정해주는 방향으로 움직임
- 라우터가 하는 라우팅을 위해 모든 네트워크 접속자는 `IP Address` 를 가지고 있어야 함
  - 네트워크 사용자가 모두 고유로 가지는 주소
  - `IP Address` 는 유동적
  - ex) 사용자가 노트북을 들고 어디를 가던 `MAC Address` 는 바뀌지 않음, 그러나 `IP Address` 의 경우 사용자가 집에서 LAN선에 연결할 때, 카페에 가서 wifi를 사용할 때 다 다름(`MAC Address` ⇒ 디바이스에 부여받는 주소, `IP Address` ⇒ 네트워크에 부여받는 주소)
- 네트워크 계층에서 수행하는 작업
  - 라우팅 - 라우팅 테이블을 통해 `Packet` 의 다음 경로를 정해주는 작업
  - 포워딩 - 라우팅 작업을 통해 결정된 경로로 데이터를 `Packet` 구조로 만들어 다음 라우터로 `Packet` 으로 전달하는 작업
- 네트워크 계층에서는 데이터링크에서 받은 `Frame` 을 `Packet` 으로 관리
- `Frame` 을 `Packet` 으로 나누고 `IP Address` 확인
- 라우터의 라우팅 테이블은 IP 주소에 따라 어떤 MAC 주소를 부여해야 할 지 정의되어 있음
- 만들어진 새로운 `Packet` 을 네트워크 카드에 전달주는 것이 네트워크 계층의 역할

### 전송 계층(Transport Layer) - TCP, UDP (segment)

- 주 목적은 양 끝에 있는 두 사용자 간에 데이터를 정확하게 전달할 수 있는가
  - 내가 전송한 데이터가 네트워크 속에서 수많은 노드를 거치더라도 데이터의 정확성이 보장되어야 함
- 네트워크 계층에서 전송한 `Packet` 이 IP 주소를 통해 원하는 네트워크에 도달했다면, `Packet` 에서 IP 부분을 제외한 `Segment` 의 적합성을 봐야 함
- 과거의 송신은 **회선 교환 방식**
  - 굳이 전송 계층이 필요 X
  - 데이터 전송을 원하는 두 지점의 회선 경로가 미리 정해져 있었고, 데이터가 중간에 바뀔 염려도 X
  ⇒ 하지만 해다 회선(경로)에 문제가 생기면 데이터를 주고받지 못하는 경우가 발생
- 현대 송신은 **패킷 교환 방식**
  - 경로가 정해지지 않은채로 데이터를 전송하면 네트워크 환경 안에서 임의의 경로(라우터에 존재하는 라우팅 테이블에서 결정해주는 정보)에 따라 움직이게 됨
  - 일부 회선이 손상되더라도 데이터가 송신된다는 장점은 있었지만 `Packet` 의 정확성 문제
  - **이러한 문제를 해결하기 위해 생겨난 프로토콜이 `TCP` 와 `UDP`**
- TCP(Transport Control Protocol, 전송 제어 프로토콜) 의 `Three Way Handshake`
  - 통신을 원하는 상대에게 통신을 원한다는 메세지를 보냄(SYN)
  - SYN을 받은 대상이 통신 준비가 되었다는 정보를 송신자에게 보냄(SYN-ACK)
  - SYN-ACK를 잘 받았다고 다시 한번 수신자에게 확인(ACK)
  ⇒ 네트워크 연결, 정확성이 보장된 데이터를 주고 받게 됨
- TCP는 데이터의 정확성 보장이 가장 큰 특징, 내가 보낸 데이터가 얼마나 전송되었고 어느 부분이 누락되었는지 판단 가능, 양자간의 정확한 전송 보장
  - 오류 복구, 흐름 복귀, 중복 제어 가능
- 반면에 UDP(User Datagram Protocol)는 신뢰성보다 **신속함**에 초점
  - IP 주소 보고 데이터를 마구잡이로 보냄
  - 쓸모없어 보이지만 미디어 전송에서 우세
  - TCP에 비해 헤더의 길이(UDP: 목적지 주소, 실데이터)가 매우 짧기 때문에 데이터 누락이 발생되었는지도 모르게 잘 보여주게 됨

### 세션 계층(Session Layer)

- 전송 계층을 통해 두 네트워크 간의 연결이 성립되면, 네트워크 세션이 생성되었다고 표현
  - TCP로 연결이 성립되면 TCP 세션
- 형성된 세션 간의 데이터 송수신 관리를 어떻게 할지 정의
  - 한번 연결된 세션에서 향후 표현 계층과 응용 계층에서 어떻게 사용자에게 정보를 보여줄 것인지
  - 형성된 세션이 끊기지 않도록 어떻게 관리할 것인지
- 세션 계층의 역할
  - 세션 설정 및 해제 방법 정의(인증 및 허가 포함)
  - 세션 다중화 / 단일화 - 통신 효율을 위해 세션을 다중화할지 단일화할지 결정
  - 송수신 방식(duplex), 반이중 방식(half-duplex), 전이중 방식(Full-duplex) 결정
  - 세션 화복
  리눅스 사용 시 원격 접속을 위해 SSH를 사용하게 되는데, SSH가 세션 계층에서 작동하는 대표적 프로토콜

### 표현 계층(Presentation Layer)

- 전달받은 네트워크 메시지를 사용자가 볼 수 있도록 만드는 기능
  - 지금까지 누군가가 보낸 메시지를 `Frame` , `Packet` , `Segment` 로 포장해서 전달했다면, 이 포장지를 뜯어 사용자에게 전달하는 것
- 표현 계층의 역할
  - 수신자 장치(노트북, 스마트폰 등)에 적합한 어플리케이션으로 노출 시킬 수 있도록 데이터 해석
  - 데이터 압축
  - 데이터 암복호화
- MIME 인코딩이나 암복호화 등의 동작이 이 계층에서 이루어짐(ex. EBCDIC로 인코딩된 문서 파일을 ASCII로 인코딩된 파일로 바꿔줌)

### 응용 계층(Application Layer)

- 우리에게 친숙한 응용 계층
- 네트워크 기능을 작동하는 모든 프로그램들은 응용 계층에서 작동(크롬, 사파리, 인터넷에 글 올리기, 게임 접속 등 우리가 네트워크로 접속하기 위한 사용자 인터페이스 의미)
- 응용 계층의 대표적 프로토콜
  ...
  - HTTP(HyperText Transfer Protocol) - www 상에서 정보를 주고받을 수 있는 프로토콜, HTML 문서를 주고 받는 역할 등
