...
- 토큰은 세션과 달리 서버가 아닌 클라이언트에 저장되기 때문에 메모리나 스토리지 등을 통해 세션을 관리했던 서버의 부담을 덜 수 있음
...
- 개방형 표준으로 안전한 통신을 지원
- 데이터는 디지털 서명을 통해 확인하고 HTTP를 통해 전송되는 경우에는 데이터를 암호화하여 안전하게 보호
- JWT 구성요소
  - `header` : 사용할 타입, 토큰 유형을 비롯해 관련된 서명(해시) 알고리즘 정의
  - `payload` : 서버에서 첨부한 사용자 권한, 데이터 (토큰 발급자, 토큰 유효기간) 등 정의
  - `signature` : 전자서명(header, payload를 해시화하고 개인키로 서명한 값)
- JWT 절차
  - 클라이언트가 사용자 아이디, 패스워드 통해 서비스 인증
  - 서버에서 서명된 JWT를 생성하여 클라이언트에 응답
  - 클라이언트가 데이터를 추가적으로 요청할 때 JWT를 HTTP header 에 첨부
  - 서버에서 클라이언트로부터 온 JWT 검증
- 장점
  - JSON 로 생성된 토큰은 용량이 작기 때문에 빠르게 전달됨
  - 거의 모든 곳에서 토큰이 생성될 수 있고 서버에서 토큰을 확인할 필요 X
  - 액세스 가능한 데이터, 권한 지속 시간, 로그인 시 가능한 작업 지정 가능
- 단점
  - 단일 키로 키가 유출되면 시스템 전체가 위험에 노출됨
  - 메시지를 모든 클라이언트에게 푸시할 수 없고 서버 측 클라이언트도 관리 X
- `Access Token` , `Refresh Token`
  - `Access Token` : 클라이언트가 갖고 있는 실제로 유저의 정보가 담긴 토큰으로, 클라이언트에서 요청이 오면 서버에서 해당 토큰에 있는 정보를 활용하여 사용자 정보에 맞게 응답 진행
  - `Refresh Token` : 새로운 `Access Token` 을 발급해주기 위해 사용하는 토큰으로 짧은 수명을 가지는 `Access Token` 에게 새로운 토큰을 발급해주기 위해 사용. 보통 디비에 유저정보와 같이 기록됨
  - JWT 인증 방식을 만약 `Access Token` 만을 이용하면, `Access Token` 은 발급된 이후 서버에 저장되지 않고 클라이언트에 저장되어 토큰 자체로 검증을 하며 사용자 권한인증을 하기 때문에 `AT` 가 탈취되면 토큰 만료 전까지 토큰을 획득한 사람은 누구나 권한 접근이 가능해지는 문제가 발생
    <br/>
    → 그래서 토큰의 유효 시간을 부여하여 탈취 문제에 대해 대응을 하기도 하지만, 유효 기간이 짧을 경우 그만큼 사용자는 로그인을 자주해야 하는 번거로움이 발생
    <br/>
    → 따라서 이러한 문제를 해결하기 위해 추가 토큰 `Refresh Token` 을 활용하여 보안을 강화
