...
- 쿠키를 기반으로 하지만 사용자 정보 파일을 브라우저에 저장하는 쿠키와는 달리 세션은 서버 측에서 관리
- 서버에서 클라이언트를 구분하기 위해 세션 ID를 부여하며 웹 브라우저가 서버에 접속해서 브라우저를 종료할 때까지 인증 상태 유지
- 접속 시간에 제한을 두어 일정 시간 응답이 없다면 정보가 유지되지 않게 함
- 사용자에 대한 정보를 서버에 두기 때문에 쿠키보다 보안 측면에서 좋지만, 사용자가 많아질수록 서버 메모리를 많이 차지함 → 동접이 많은 경우 서버에 과부하를 주게되고 성능 저하
- 클라이언트가 Request를 보내면, 해당 서버의 엔진이 클라이언트에게 유일한 ID를 부여하는데 이것이 세션 ID

### 세션 동작 방식

1. 클라이언트가 접속 시 서버에서 세션 ID 발급
...
- 각 클라이언트에게 고유 ID 부여
- 세션 ID로 클라이언트를 구분하여 클라이언트의 요구에 맞는 서비스 제공
- 보안 면에서 쿠키보다 우수
